...
struct sRGB_LED RGBleds[NB_RGB] = {0};        /* array of leds */
 struct cRGB bufferLeds[NB_RGB] = {0};  /* buffer of data to send (dedicated to light_ws2812 libray) */
...
uint16_t blinkEventTable[NB_RGB] = {0};    /* intermediate table: the same as blinkEvent, but for each led. Use only to do not have to recompute it every time (we have here enough bytes in RAM for this) */
...
void updateRGB()
{
	ws2812_setleds(bufferLeds, NB_RGB);
	RGBledsHasChanged = 0;
}
